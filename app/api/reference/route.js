...
// A single global style-reference image (base64 PNG/JPEG, no data-URL prefix).
// When set, new generations use it as a style anchor.
...
export async function POST(req) {
  try {
    ...
    await setSetting("reference_image", b64);
...
export async function DELETE() {
  try {
    await initSchema();
    await setSetting("reference_image", "");
