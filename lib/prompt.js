...
// SHINY — a rarity-independent special variant: ONE palette-shifted image per pet.
// Same image-to-image edit as the tiers, but a full colour-palette shift (not a
// subtle shimmer, so it reads even on already-white/arctic pets). Two sets, picked
// by the Matt/Shiny (fur vs metal) selector, same as the tier edits.
export const SHINY_EDIT_A =
  "keep the exact same character, pose, markings pattern, eye color and all shapes; this is the special SHINY variant, so SHIFT THE ENTIRE COLOR PALETTE to a clearly different but harmonious alternate scheme — a big, obvious color change, not a subtle shimmer. If the base is warm, go cool; if dark, go luminous/pearly; if already pale/white, shift to a distinct new tint (icy blue, rose-gold, lilac, etc) so there is ALWAYS a strong visible color change. Keep the markings pattern in place but recolor them; keep the eyes their original color for recognition; add a light iridescent sheen. It must clearly read as a striking alternate-color shiny of the same creature. CRITICAL: do not redraw or reshape the face, eyes, mouth, nose/beak, feet/paws or any body part — keep the anatomy, pose and every shape identical to the base; only recolor and add the sheen. Keep everything within the silhouette so it crops cleanly to transparent.";
export const SHINY_EDIT_B =
  "keep the exact same character, pose, markings pattern, eye color and all shapes; this is the special SHINY variant, so SHIFT THE ENTIRE COLOR PALETTE of the hard shiny surfaces (armor, plating, shell, crystal, gems) to a clearly different but harmonious alternate metal scheme — a big, obvious color change, not a subtle shimmer. If the base metal is one color, shift it to a distinct new metallic tint (e.g. steel→rose-gold, gold→iridescent violet-steel) so there is ALWAYS a strong visible change; recolor any skin/fur/beak to a matching alternate tint too. Keep the eyes their original color for recognition; add a light iridescent sheen on the metal. It must clearly read as a striking alternate-color shiny of the same creature. CRITICAL: do not redraw or reshape the face, eyes, mouth, nose/beak, feet/paws or any body part — keep the anatomy, pose and every shape identical to the base; only recolor and add the sheen; keep the shield/emblem shape intact. Keep everything within the silhouette so it crops cleanly to transparent.";
