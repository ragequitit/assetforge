...
// --- OpenAI (GPT Image) ---------------------------------------------------
// Defaults to gpt-image-1.5: newer/higher quality than gpt-image-1, cheaper at
// every tier, and it still returns a native transparent PNG (which gpt-image-2
// does NOT) — so the crop/centre/downscale pipeline is unchanged. Override with
// IMAGE_MODEL to roll back (e.g. IMAGE_MODEL=gpt-image-1) without a code change.
// It only renders square at 1024x1024, so we always request that and let the
// Python post-processor normalise to the chosen export size.
async function generateOpenAI(prompt, opts = {}) {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) throw new Error("OPENAI_API_KEY saknas i .env.local");

  const model = opts.model || process.env.IMAGE_MODEL || "gpt-image-1.5";
...
// positive, flowing style language before it reaches the image model. A low
...
// --- Image-to-image edit (rarity tiers from a base image) -----------------
// Runs a true edit against the base image so the pet's identity is preserved and
// the tier only layers on glow/finish. Always uses a transparent-capable model
// (IMAGE_EDIT_MODEL, default gpt-image-1.5 — gpt-image-2 can NOT do transparent
// output). input_fidelity=high keeps the source's details. The instruction is
// sent RAW (the prompt enricher is intentionally NOT used here). `base` is a PNG
// Buffer. Returns a transparent PNG Buffer.
export async function editImage(base, instruction, opts = {}) {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) throw new Error("OPENAI_API_KEY saknas i .env.local");
  const model = opts.model || process.env.IMAGE_EDIT_MODEL || "gpt-image-1.5";
  const quality = opts.quality || process.env.IMAGE_QUALITY || "high";

  const bytes = Buffer.isBuffer(base) ? base : Buffer.from(base);
  const form = new FormData();
  form.append("model", model);
  ...
  form.append("background", "transparent");
  form.append("quality", quality);
  form.append("input_fidelity", "high"); // hold the base's identity/details
  form.append("image", new Blob([bytes], { type: "image/png" }), "base.png");
  ...
  if (!res.ok) {
    const detail = await res.text();
    throw new Error(`OpenAI edit-fel (${res.status}): ${detail}`);
  }
  const data = await res.json();
  const b64 = data?.data?.[0]?.b64_json;
  if (!b64) throw new Error("OpenAI returnerade ingen redigerad bild.");
  return Buffer.from(b64, "base64");
}
