...
export async function initSchema() {
  ...
  await p.query(`
    ...
    -- Rarity-tiers-from-base jobs: kind='edit' means "run an image-to-image edit
    -- on source_image using edit_prompt (a raw tier instruction), then process".
    -- An empty edit_prompt = the Common tier (copy the base straight through, no
    -- API call). These live in their own gallery, separate from generate/import.
    ALTER TABLE jobs ADD COLUMN IF NOT EXISTS edit_prompt TEXT;

    -- kind='edit_base' is the hidden first step of that flow: it cleans ONE
    -- uploaded base to a transparent, centred PNG (so a colored-background photo
    -- works too), then fans out one edit job per chosen tier from that clean base.
    -- edit_plan holds the chosen tiers + variant count as JSON for the fan-out.
    ALTER TABLE jobs ADD COLUMN IF NOT EXISTS edit_plan TEXT;

    -- Background-removal method for import jobs: 'auto'|'rembg'|'floodfill'.
    -- floodfill is cleanest for flat, solid backgrounds behind an outlined subject.
    ALTER TABLE jobs ADD COLUMN IF NOT EXISTS bg_method TEXT;
  `);
