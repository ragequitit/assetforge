...
import { CATEGORIES, RARITIES, SIZES, assetFilename } from "./prompt.js";

// Build case-insensitive name lookups from the loadout's own vocabulary, falling
// back to the built-in lists if none were provided.
function buildLookups(defaults) {
  const cats = Array.isArray(defaults.categoryNames) && defaults.categoryNames.length
    ? defaults.categoryNames
    : CATEGORIES;
  const rars = Array.isArray(defaults.rarityNames) && defaults.rarityNames.length
    ? defaults.rarityNames
    : RARITIES;
  return {
    cat: new Map(cats.map((c) => [c.toLowerCase(), c])),
    rar: new Map(rars.map((r) => [r.toLowerCase(), r])),
  };
}
...
export function parseLine(rawLine, defaults, lookups) {
  const line = rawLine.trim();
  if (!line || line.startsWith("#")) return null;

  const lk = lookups || buildLookups(defaults);
  ...
  if (parts[1]) {
    const m = lk.cat.get(parts[1].toLowerCase());
    if (m) category = m;
  ...
  if (parts[2]) {
    const m = lk.rar.get(parts[2].toLowerCase());
    if (m) rarity = m;
...
export function parseBatch(text, defaults) {
  ...
  const items = [];
  let warningCount = 0;
  const lookups = buildLookups(defaults);

  for (const raw of lines) {
    const item = parseLine(raw, defaults, lookups);
    if (item) {
      items.push(item);
